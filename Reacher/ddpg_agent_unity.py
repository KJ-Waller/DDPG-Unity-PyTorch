...
class DDPGAgent(object):
    def __init__(self, alpha, beta, input_dims, tau, env, brain_name, gamma=.99,
                 n_actions=2, mem_capacity=1e6, l1_size=1000, l2_size=800, 
                 l3_size=400, batch_size=64, multiagent=False, n_agents=None, 
                 game_name='Walker'):
        ...
        self.noise = [OUActionNoise(np.zeros(n_actions)) for i in range(n_agents)] if multiagent else \
                    OUActionNoise(np.zeros(n_actions))

        # Setup device used for torch computations
        self.device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')

        # Create actor critic and target networks
        self.actor = ActorNet(alpha, input_dims, l1_size, l2_size, l3_size, n_actions, 
                            name='actor_' + game_name + '_ddpg_model').to(self.device)
        self.target_actor = ActorNet(alpha, input_dims, l1_size, l2_size, l3_size, n_actions).to(self.device)

        self.critic = CriticNet(beta, input_dims, l1_size, l2_size, l3_size, n_actions, 
                                name='critic_' + game_name + '_ddpg_model').to(self.device)
        self.target_critic = CriticNet(beta, input_dims, l1_size, l2_size, l3_size, n_actions).to(self.device)
